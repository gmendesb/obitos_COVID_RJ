...
Por que bairros como Copacabana e Campo Grande são os que apresentam os mais altos números de óbito por COVID-19? A mortalidade nesses bairros seria realmente mais alta que em outras regiões do município? Ou seria porque estes são bairros mais populosos? Ou ainda porque têm uma maior concentração de idosos em suas populações? As análises nessa nota tentam responder tais perguntas.

# Dados

## Base de Dados sobre casos de COVID-19

O [Painel Rio COVID-19](https://experience.arcgis.com/experience/38efc69787a346959c931568bd9e2cc4) apresenta o resumo do número de casos confirmados, óbitos, entre outras informações, bem como a opção de download dos microdados[^1]. Esta é uma importante iniciativa, já que permite o estudo da epidemia via informações bastante detalhadas, e espera-se que seja mantida e aprimorada.

[^1]: [O número de óbitos deixou de aparecer no painel na dia 18/05/2020](https://oglobo.globo.com/rio/prefeitura-do-rio-tira-do-ar-dados-abertos-sobre-mortos-pela-covid-19-no-municipio-24434182), sob a justificativa que que está havendo uma [mudança metodológica](https://valor.globo.com/brasil/noticia/2020/05/25/rio-muda-metodologia-de-divulgacao-de-mortes-por-covid-19.ghtml) na classificação dos óbitos. Ainda assim, foi possível extrair tais informações via microdados até o dia 24/05/2020.

Os microdados contêm dados individuais dos casos confirmados de COVID-19 no município do Rio de Janeiro, com informações relevantes, como bairro de residência ou estadia do paciente, seu sexo, faixa etária e evolução do caso (ativo, óbito ou recuperado).

Esta nota avalia a mortalidade por COVID-19 e, portanto, utiliza os casos que evoluíram para óbito.

Com o objetivo de permitir a reprodutibilidade dos resultados, todos os códigos utilizados para produzir os resultados desta nota estão também disponíveis. Espera-se que isso também ajude outros pesquisadores que pretendam usar as mesmas bases de dados.
...
list.of.packages <- c("tidyverse", "gt", "geobr", "ggsflabel", "knitr", "kableExtra", "sidrar", "mipfp")
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)) install.packages(new.packages)
invisible(lapply(list.of.packages, library, character.only = TRUE))

# lê base atualizada do site Painel Rio COVID-19
# dados_COVID_Rio <- read.csv2("http://monitoramento.subpav.rio/COVID19/dados_abertos/Dados_indiv_MRJ_covid19.csv",encoding = "latin1", sep = ";", stringsAsFactors = F)
dados_COVID_Rio <- read_csv2(
  file = "http://monitoramento.subpav.rio/COVID19/dados_abertos/Dados_indiv_MRJ_covid19.csv",local = locale(encoding = "latin1"),
...
# agrupa idade 80+ e bairros Gericinó e Vasco da Gama, que foram criados entre 2000 e 2010
...
$TEM = \frac{O_{i,s}}{P_i{i,s}}$,

onde $O_i$ é o número de óbitos no grupo etário $i$ e sexo $s$ e $P_i$ é a população no mesmo no grupo etário $i$ e sexo $s$.

As estimativas serão atualizadas para considerar estruturas populacionais mais realistas. Como primeira aproximação, adotou-se a estrutura etária observada em 2010 aplicada aos totais populacionais estimados para 2020. Pretende-se utilizar métodos que adotem hipótese mais realistas em futuras atualizações desta nota, mas acredita-se que as panorama geral dos resultados devam se manter.

```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
# faz merge das duas bases de população
base_pop_idade <- inner_join(pop_bairro_Rio %>% filter(ano==2010), 
                             estima_pop_tot, by = c("bairro", "bairro")) %>% 
...
O gráfico abaixo mostra as Taxas Específicas de Mortalidade por sexo e idade (em escala logaritmica) para o conjunto do município do Rio de Janeiro, mostrando uma curva de mortalidade aparentemente próxima do que se conhece para a mortalidade total.
...
O gráfico abaixo mostra as Taxas Específicas de Mortalidade (em escala logarítmica) por idade a partir dos 30 anos para os bairros de Copacabana e Rocinha. Nota-se que a mortalidade nos dois bairros é próxima (e relativamente baixa) até os 59 anos, mas significativamente mais alta na Rocinha entre os idosos.
...
ggplot(base_idade %>% 
  ...
  geom_point(aes(group=bairro, color = bairro, shape = bairro)) +
  scale_y_log10() +
...
## Taxa Bruta de Mortalidade Padronizadas

Uma das desvantagens da Taxas Específicas de Mortalidade (TEM) para comparações dos níveis de mortalidade é que é preciso fazer comparações simultâneas de vários grupos de idade e sexo. 
...
Observa-se que os bairros de Campo Grande, Bangu, Realengo, que estão na lista dos que tiveram o maior número absoluto de óbitos, também não figuram na lista dos que têm maior taxa de mortalidade padronizada, já que são bairros bastante populosos. Copacabana também não aparece mais no topo da lista, com $TBM = $`r unlist(TBM_pad %>% filter(bairro=="COPACABANA") %>% select(TBM_pad))` por 100 mil habitantes. Por outro lado, bairros como Rocinha e Cidade de Deus passam a figurar entre aqueles com maior taxa de mortalidade.
...
obitos <- ggplot(data=setorRJ) +
  geom_sf(aes(fill=log10(obitos)), color= "white", size=.005) +
  theme_void() +
  coord_sf() +
  scale_fill_gradientn(name = "log(TBM)",
                       colours = c("white","yellow", "gold", "orange", "red", "red4"),
                       na.value = 'grey95',
                       breaks=seq(0,5,by=.5),
...
mapaTBM <- ggplot(data=setorRJ) +
  geom_sf(aes(fill=log10(TBM)), color= "white", size=.005) +
  theme_void() +
  coord_sf() +
  scale_fill_gradientn(name = "log(TBM)",
                       colours = c("white","yellow", "gold", "orange", "red", "red4"),
...
mapaTBM_pad <- ggplot(data=setorRJ) +
  geom_sf(aes(fill=log10(TBM_pad)), color= "white", size=.005) +
  theme_void() +
  coord_sf() +
  scale_fill_gradientn(name = "log(TBM)",
                       colours = c("white","yellow", "orange", "red", "red4"),
                       na.value = 'grey95',
                       breaks=seq(0,3,by=.5),
                       guide = guide_colourbar(title.position = "top", title.hjust = 1)) +
  theme(strip.text.x = element_text(size = 12, colour = "black", face = "bold"),
...
Tais análises só foram possíveis devido à disponibilidade dos dados de óbitos por COVID-19 com bom detalhamento, com informação por bairro, sexo e idade. Informações adicionais, como data do óbito, permitiriam análises ainda mais precisas. Espera-se, desta forma, que se reestabeleça as informações sobre o número de óbitos do painel da Prefeitura do Rio na internet, [retirado recentemente](https://g1.globo.com/rj/rio-de-janeiro/noticia/2020/05/22/crivella-tira-informacoes-sobre-obitos-da-covid-19-de-plataforma-online-e-diz-que-protocolo-de-contagem-no-rio-sera-revisto.ghtml), para seguir dando transparência às informações.
